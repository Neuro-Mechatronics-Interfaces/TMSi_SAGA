classdef RealTimePlot_EMGProcessed < TMSiSAGA.HiddenHandle
    ...
    methods
        ...
        function append(obj, samples)
            ...
            % Process EMG
            fs = 4000; % sample rate
            fcl = 10/(fs/2); % high pass range = 10 Hz 
            fch = 1000/(fs/2); % low pass range = 1000 Hz
            [b,a] = butter(1, [fcl fch]); % bandpass filter
            filtsig = filtfilt(b,a,samples); % filter signal
            rect_x = abs(filtsig); % rectify signal
            samples = envelope(rect_x,10000,'rms'); % Smooth signal
