...
try
    ...
    mean_max_intensity = max(abs(data.samples(1, interval)));
...
%     Create a real time plot
    ...
    rPlot.show();
    data2 = TMSiSAGA.Data('Plot', device.sample_rate, {struct('alternative_name', 'Intensity', 'unit_name', '%')});
    % Start sampling on the device
    device.start();
    ...
    while rPlot.is_visible
        ...
        if num_sets > 0
            ...
            samples = abs(samples(1, :)) ./ mean_max_intensity .* 100;
